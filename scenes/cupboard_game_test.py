...
class TilemapRenderer:
    def __init__(self, tileset_path, tilemap_path, tile_size=32):
        ...
        self.last_key = None  # Track last key pressed
        self.key_count = 0  # Count successful key presses
        self.required_presses = 5  # Number of presses needed
        self.can_move = False  # Whether player can move
        self.move_direction = None  # Direction to move when ready
        self.key_was_released = True  # Track if key was released between presses
        
        # Energy bar properties
        self.energy_bar_width = 200
        self.energy_bar_height = 20
        self.energy_bar_color = (0, 255, 0)  # Green
        self.energy_bar_bg_color = (100, 100, 100)  # Gray
    ...
    def handle_player_movement(self, key):
        ...
        if not self.can_move:
            print("Need to press the same key 5 times first!")
            return
        ...
        if (0 <= new_x < self.map_width and 0 <= new_y < self.map_height):
            ...
            print("Movement complete! Press and hold a key again to move.")
        else:
    ...
    def draw_key_progress(self):
        ...
        if self.last_key is not None:
            key_name = self.get_direction_name(self.last_key)
            status = f"Pressing {key_name}: {self.key_count}/{self.required_presses}"
            color = (255, 255, 255)
        else:
            status = "Press and release W/A/S/D 5 times in a row"
            color = (255, 255, 255)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if self.key_was_released:  # Only count press if key was released
                            if self.last_key is None or event.key == self.last_key:
                                # Same key or starting new sequence
                                self.last_key = event.key
                                if self.key_was_released:  # Only increment if key was released
                                    self.key_count += 1
                                    print(f"Appuyez {self.key_count}/5")
                                ...
                                # Different key pressed, reset sequence
                                print("Sequence broken! Start over.")
                                self.last_key = None
                                self.key_count = 0
                                self.can_move = False
                            self.key_was_released = False  # Key is now pressed
                
                elif event.type == pygame.KEYUP:
                    if event.key in [pygame.K_w, pygame.K_a, pygame.K_s, pygame.K_d]:
                        self.key_was_released = True  # Key is now released

            # Clear and draw
            self.screen.fill((0, 0, 0))
            self.draw_tilemap()
            self.draw_player()
            self.draw_key_progress()  # Draw key progress bars
